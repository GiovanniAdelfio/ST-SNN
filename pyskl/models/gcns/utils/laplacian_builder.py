...
class LaplacianBuilder(nn.Module):
    def __init__(self, size, edge_index, d, normalised=False, deg_normalised=False, add_hp=False, add_lp=False,
        ...
        # Preprocess the sparse indices required to compute the Sheaf Laplacian.
        full_idx, _ = compute_left_right_map_index(edge_index, full_matrix=True)
        lr_idx, vertex_idx = compute_left_right_map_index(edge_index)
        
        self.register_buffer('full_left_right_idx', full_idx)
        self.register_buffer('left_right_idx', lr_idx)
        self.register_buffer('vertex_tril_idx', vertex_idx)
        
        if self.add_lp or self.add_hp:
            fd_idx, ft_idx = compute_fixed_diag_laplacian_indices(
                size, self.vertex_tril_idx, self.d, self.final_d)
            self.register_buffer('fixed_diag_indices', fd_idx)
            self.register_buffer('fixed_tril_indices', ft_idx)
            
        self.register_buffer('deg', degree(self.edge_index[0], num_nodes=self.size))
...
class GeneralLaplacianBuilder(LaplacianBuilder):
    ...
    def __init__(self, size, edge_index, d, normalised=False, deg_normalised=False,
        ...
        # Preprocess the sparse indices required to compute the Sheaf Laplacian.
        d_idx, t_idx = compute_learnable_laplacian_indices(
            size, self.vertex_tril_idx, self.d, self.final_d)
        self.register_buffer('diag_indices', d_idx)
        self.register_buffer('tril_indices', t_idx)

    def normalise(self, diag_maps, non_diag_maps, tril_row, tril_col):
        if self.normalised:
            # Estrai il device dinamico e corretto in questo preciso istante
            current_device = diag_maps.device
            
            # Normalise the entries if the normalised Laplacian is used.
            if self.training:
                eps = torch.FloatTensor(self.d).uniform_(-0.001, 0.001).to(device=current_device)
            else:
                eps = torch.zeros(self.d, device=current_device)
    ...
    def forward(self, maps):
        ...
        current_device = maps.device
        row = self.edge_index[0].to(current_device)
        ...
        triu_indices = torch.empty_like(tril_indices, device = current_device)
